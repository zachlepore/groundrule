...
insert into public.rule_outcomes(rule_version_id,sequence,outcome_type,parameters,severity,message_template)
select rv.id,1,v.outcome,v.parameters,v.severity,v.message from public.rules r join public.rule_sets rs on rs.id=r.rule_set_id join public.rule_versions rv on rv.rule_id=r.id join (values
('permit.small_shed_exemption','permit_threshold','{"exempt_max_sq_ft":100,"exempt_max_inclusive":true,"permit_required_min_exclusive_sq_ft":100}'::jsonb,'advisory','A shed of 100 square feet or smaller does not require a building permit; City development standards still apply.'),
('permit.larger_shed_review','external_authority_required','{"trigger_min_exclusive_sq_ft":100}'::jsonb,'warning','If the shed is larger than 100 sq ft, contact Clearwater Permitting for the building-permit path.'),
('permit.utilities','external_authority_required','{"trades":["electrical","plumbing"]}'::jsonb,'warning','If the shed will have electricity or plumbing, confirm the required trade permits with Clearwater.'),
('location.lmdr_setbacks','minimum_setbacks','{"front_ft":25,"side_ft":5,"rear_ft":10,"measured_from":"property_line"}'::jsonb,'requirement','Keep the shed at least 25 feet from the front property line, five feet from a side property line, and ten feet from the rear property line.'),
('height.residential_maximum','maximum','{"value":15,"unit":"ft"}'::jsonb,'requirement','The base maximum accessory-structure height is 15 feet.')
) v(key,outcome,parameters,severity,message) on v.key=r.key where rs.key='clearwater_shed_v1' and rv.version_number=1 and rv.published_at is null on conflict(rule_version_id,sequence) do nothing;

insert into public.rule_version_inputs(rule_version_id,input_definition_id,role,authority_requirement,notes)
select rv.id,d.id,'applicability',case when d.authoritative_source_required then 'authoritative' else 'none' end,v.notes
...
join (values
('permit.small_shed_exemption','project.structure_type','Derived from selecting the shed workflow.'),
('permit.larger_shed_review','project.structure_type','Derived from selecting the shed workflow.'),
('permit.utilities','project.structure_type','Derived from selecting the shed workflow.'),
('location.lmdr_setbacks','project.structure_type','Derived from selecting the shed workflow.'),
('location.lmdr_setbacks','property.zoning_district','Property-derived zoning determines applicability.'),
('height.residential_maximum','project.structure_type','Derived from selecting the shed workflow.'),
('height.residential_maximum','property.zoning_district','Property-derived zoning determines applicability.')
) v(rule_key,input_key,notes) on v.rule_key=r.key
...
begin
  ...
  if version_count <> 5 or outcome_count <> 5 or citation_count <> 5 or input_count <> 7 then

...
export function preprocess(data, snapshots, pilot = { jurisdiction: "clearwater-fl" }) {
  ...
  for (const parcel of parcels) if (parcel.id != null) parcelsById.set(String(parcel.id), [...(parcelsById.get(String(parcel.id)) ?? []), parcel]);
  ...
  const profiles = data.addresses.features.map((address) => {
    ...
    let parcelMatches = sourceParcelId ? (parcelsById.get(String(sourceParcelId)) ?? []) : [], parcelMatchMethod = parcelMatches.length ? "authoritative_identifier" : "spatial_covers";
    ...
    if (address.geometry?.type !== "Point") issues.push("invalid_address_geometry");
    ...
    if (parcel && !point) issues.push("invalid_parcel_geometry");
  ...
  const percent = (count) => profiles.length ? Number((count * 100 / profiles.length).toFixed(2)) : 0;
  const review = profiles.length - clean.length;
  const stats = { pilotName: pilot.pilotName, addressesFetched: data.addresses.features.length, uniqueNormalizedAddresses: duplicateCounts.size, uniqueParcelsRepresented: parcelSet(() => true).size, parcelsFetched: parcels.length, zoningPolygonsFetched: zoning.length, clearwaterConfirmedProfiles: profiles.filter((p) => p.normalizedJurisdiction === "clearwater" && p.jurisdictionStatus === "confirmed").length, cleanPropertyProfiles: clean.length, cleanPropertyProfilesPercentage: percent(clean.length), cleanProperties: new Set(clean.map((p) => p.parcelIdentifier)).size, reviewPropertyProfiles: review, reviewPropertyProfilesPercentage: percent(review), reviewProperties: new Set(profiles.filter((p) => p.status === "review").map((p) => p.parcelIdentifier).filter(Boolean)).size, unmatchedAddresses: countIssues("address_without_parcel"), missingParcels: countIssues("address_without_parcel") + countIssues("missing_parcel_identifier"), multipleParcelCandidates: countIssues("address_multiple_parcels"), ambiguousAddresses: profiles.filter((p) => p.issues.includes("duplicate_address") || p.issues.includes("address_multiple_parcels")).length, duplicateCanonicalAddresses: countIssues("duplicate_address"), missingZoning: countIssues("parcel_without_zoning"), conflictingZoning: countIssues("parcel_ambiguous_zoning"), jurisdictionConflicts: countIssues("conflicting_address_municipality") + countIssues("missing_address_municipality"), geometryFailures: countIssues("invalid_address_geometry") + countIssues("invalid_parcel_geometry"), otherValidationFailures: profiles.filter((p) => p.issues.some((issue) => ["missing_address_identity", "unsupported_zoning_code"].includes(issue))).length, duplicateOrAmbiguousAddressMatches: profiles.filter((p) => p.issues.includes("duplicate_address") || p.issues.includes("address_multiple_parcels")).length, parcelsWithNoZoning: parcelSet((p) => p.issues.includes("parcel_without_zoning")).size, parcelsWithAmbiguousZoning: parcelSet((p) => p.issues.includes("parcel_ambiguous_zoning")).size, zoningDistrictsRepresented: [...new Set(profiles.map((p) => p.zoningCode).filter(Boolean))].sort(), evaluatorReadyZoningDistrictProfiles: evaluatorReady.length, evaluatorReadyZoningDistrictPercentage: percent(evaluatorReady.length), matchMethodsUsed: countBy(profiles.map((p) => p.parcelMatchMethod)), issueCountsByType: countBy(profiles.flatMap((p) => p.issues)), rawMunicipalityAddressCounts: countBy(profiles.map((p) => p.rawMunicipality || "<blank>")), normalizedJurisdictionPropertyCounts: countBy(propertyResolutions.map((p) => p.normalizedJurisdiction)), jurisdictionDerivationPropertyCounts: countBy(propertyResolutions.map((p) => p.derivationMethod)) };

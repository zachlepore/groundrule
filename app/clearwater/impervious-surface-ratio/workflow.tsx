...
export function ImperviousSurfaceRatioWorkflow({initialAddress="",openProject=false}:{initialAddress?:string;openProject?:boolean}) {
 ...
 {(guide)=><article className="guide"><aside className="property-context" aria-label="Property facts used"><ul>{guide.propertyContext.map(f=><li key={f}>{f}</li>)}</ul></aside><section className={`isr-answer isr-answer-${guide.status}`}><p className="eyebrow">Impervious surface ratio</p><h2>{guide.heading}</h2>{guide.maximumPercent!==null&&<p className="isr-value">{guide.maximumPercent}%</p>}<p>{guide.explanation}</p><p className="isr-scope">{guide.scopeNote}</p><Source guide={guide}/></section><section className="isr-education" aria-labelledby="isr-what-counts"><h2 id="isr-what-counts">What counts as impervious?</h2><p>{guide.whatCounts}</p></section></article>}

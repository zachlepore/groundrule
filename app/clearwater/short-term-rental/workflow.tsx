...
export function ShortTermRentalWorkflow({initialAddress="",openProject=false}:{initialAddress?:string;openProject?:boolean}) {
 return <ClearwaterResidentShell activeGuide="short-term-rental" guideTitle="Short-term rental guidance" initialAddress={initialAddress} openProject={openProject} lookup={startShortTermRentalLookup}>
 {(guide)=><article className="guide">{guide.propertyContext.length>0&&<aside className="property-context" aria-label="Property facts used"><ul>{guide.propertyContext.map(fact=><li key={fact}>{fact}</li>)}</ul></aside>}<section className={`str-answer str-answer-${guide.status}`}><p className="eyebrow">Short-term rental</p><h2>{guide.heading}</h2><p>{guide.explanation}</p><Source guide={guide}/></section></article>}

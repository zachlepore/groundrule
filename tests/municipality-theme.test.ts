...
test("Setbacks regulatory answers and quiet trust treatment remain intact", () => {
  ...
  for (const copy of ["Building something else?", "Specific situations", "Based on Clearwater’s current development standards", "Groundrule uses official City of Clearwater codes and property data to generate this guidance.", "View Clearwater codes", "← New search"]) assert.match(workflow, new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));

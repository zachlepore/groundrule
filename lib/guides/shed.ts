...
export interface ShedGuideItem { key: string; title: string; answer: string; qualification?: string; action?: { label: string; url: string }; citations: Citation[]; values?: Record<string, JsonValue> }
...
const CLEARWATER_PERMITTING_URL = "https://www.myclearwater.com/Business-Development/Permitting";
...
export function buildClearwaterShedGuide(result: EvaluationResult, facts: Facts): ShedGuide {
  ...
  const impervious = byKey.get("site.impervious_surface");
  const exemption = byKey.get("permit.small_shed_exemption"); const threshold = outcome(exemption, "exemption");
  if (exemption && threshold) highlights.push(item(exemption, "Building permit", `Not required for sheds ${threshold.parameters.exempt_max_sq_ft} sq ft or smaller.`, `Permit exempt does not mean rule exempt. Measure the footprint using exterior dimensions; setbacks${impervious ? " and impervious-surface limits" : ""} still apply.`, threshold.parameters));
  const setback = byKey.get("location.lmdr_setbacks"); const minimums = outcome(setback, "obligation");
  if (setback && minimums) highlights.push(item(setback, "Setbacks", `${minimums.parameters.front_ft} ft from the front property line · ${minimums.parameters.side_ft} ft from a side property line · ${minimums.parameters.rear_ft} ft from the rear property line`, "The shed cannot go between the house and a street.", minimums.parameters));
  ...
  const roof = byKey.get("materials.permanent_roof"); if (roof) specificSituations.push(item(roof, "Roof material", roof.outcomes[0]?.messageTemplate ?? roof.summary));
